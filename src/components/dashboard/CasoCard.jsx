...
const statusConfig = {
  ...
  'geracao_dae': { label: 'Geração DAE', color: 'bg-amber-100 text-[#FFC107] border-[#FFC107]' },
...
export default function CasoCard({ caso }) {
  ...
  return (
    <Card className="bg-white border-2 border-slate-200 card-shadow-hover group overflow-hidden relative">
      <CardHeader className="border-b-2 border-slate-200 pb-3 sm:pb-4 p-4 sm:p-6 relative">
        <div className="flex justify-between items-start gap-2">
          <div className="flex-1 min-w-0">
            <CardTitle className="text-base sm:text-lg lg:text-xl font-bold text-[#333333] mb-2 group-hover:text-[#4169E1] transition-colors line-clamp-2">
              {caso.nome_falecido}
            </CardTitle>
      ...
      <CardContent className="pt-4 sm:pt-6 p-4 sm:p-6 space-y-3 sm:space-y-4 relative">
        <div className="grid grid-cols-2 gap-2 sm:gap-4">
          <div className="p-3 sm:p-4 bg-amber-50 rounded-lg sm:rounded-xl border-2 border-[#FFC107]">
            <div className="flex items-center gap-1 sm:gap-2 mb-1">
              <TrendingUp className="w-3 h-3 sm:w-4 sm:h-4 text-[#FFC107]" />
              <p className="text-[10px] sm:text-xs font-bold text-[#FFC107] uppercase tracking-wider">Patrimônio</p>
            </div>
            <p className="text-sm sm:text-base lg:text-lg font-extrabold text-[#FFC107] break-words">
